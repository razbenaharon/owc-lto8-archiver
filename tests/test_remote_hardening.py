...
class RemotePasswordSafetyTests(unittest.TestCase):
    ...
    def test_remote_tar_permission_denied_warning_is_recoverable(self):
        line = (
            "tar: D/shared-sets/suture_1/mvsanywhere/scans/scan2/images/"
    ...
    def test_remote_tar_non_permission_open_warning_stays_fatal(self):
        line = (
            "tar: D/shared-sets/project/file.bin: "
...
class RemoteScannerTests(unittest.TestCase):
    ...
    def test_truncated_record_outside_scan_roots_is_rejected(self):
        ...
        result = SimpleNamespace(
            returncode=0,
            stdout="10 /vault/E/data/ok.txt\0931839 /strg",
            stderr="",
        )
        empty = SimpleNamespace(returncode=0, stdout="", stderr="")
        with mock.patch("src.scanning._ssh_run",
                        side_effect=[result, empty]):
            manifest = scanner.scan(["/vault/E/data", "/vault/D/data"])
        self.assertEqual(manifest, [("/vault/E/data/ok.txt", 10)])
        self.assertEqual(tracker.count(), 1)
        item = tracker.items()[0]
