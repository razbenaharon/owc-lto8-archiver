...
class BaselineTests(unittest.TestCase):
    def test_remote_script_uses_find_type_f(self):
        srv = ServerConfig("srv01", "srv01.x", "u", "", ["/vault/D"])
        script = baseline_lib._remote_baseline_script(srv, 2)
        self.assertIn("find /vault/D -xdev -type f", script)
    ...
    def test_parse_counts_ignores_markers(self):
        text = "\n".join([
            ...
            "##### MOUNT: /vault/D #####",
            "1323147\t/vault/D/shared-sets/jigsaws",
            "5\t/vault/D",
            "##### END #####",
        ])
        counts = baseline_lib.parse_baseline_counts(text)
        self.assertEqual(counts["/vault/D/shared-sets/jigsaws"], 1323147)
        self.assertEqual(counts["/vault/D"], 5)

    def test_write_then_load_round_trips(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "coverage_baseline.json")
            baseline_lib.write_baseline(
                {"srv01": {"/vault/D/shared-sets/jigsaws": 1323147}}, path)
            loaded = baseline_lib.load_baseline(path)
        self.assertEqual(loaded["srv01"]["/vault/D/shared-sets/jigsaws"], 1323147)

    def test_load_missing_file_is_empty(self):
        self.assertEqual(baseline_lib.load_baseline("/no/such/baseline.json"), {})


# A rawlog that includes the du --inodes pass. shared-sets/op holds ~1M small
...
SAMPLE_RAWLOG_WITH_INODES = "\n".join([
    ...
    "##### MOUNT: /vault/D #####",
    "##### DF: 2199023255552 1099511627776 1099511627776 50% #####",
    f"{12 * 1024**3}\t/vault/D/shared-sets/op",
    f"{12 * 1024**3}\t/vault/D/shared-sets",
    f"{12 * 1024**3}\t/vault/D",
    "##### INODES #####",
    "1000000\t/vault/D/shared-sets/op",
    "1000010\t/vault/D/shared-sets",
    "1000012\t/vault/D",
    "##### END #####",
    "",
])
...
class InodeParsingTests(unittest.TestCase):
    def test_inode_counts_attach_to_matching_nodes(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "srv01.rawlog")
            with open(path, "w", encoding="utf-8") as fh:
                fh.write(SAMPLE_RAWLOG_WITH_INODES)
            res = parse_raw_log(path, server_name="srv01")
        mount = res.mounts[0]
    ...
    def test_dirs_section_gives_exact_file_count(self):
        log = "\n".join([
            "# server: srv01.x", "# depth: 2",
            "##### MOUNT: /vault/D #####",
            f"{12 * 1024**3}\t/vault/D/shared-sets/op",
            f"{12 * 1024**3}\t/vault/D",
            "##### INODES #####",
            "1000000\t/vault/D/shared-sets/op", "1000005\t/vault/D",
            "##### DIRS #####",
            "120\t/vault/D/shared-sets/op", "130\t/vault/D",
            "##### END #####", "",
        ])
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "srv01.rawlog")
        ...
        op = next(c for c in res.mounts[0].root.children
                  if c.path == "/vault/D/shared-sets/op")
...
class BuildCoverageTests(unittest.TestCase):
    ...
    def test_statuses_and_cumulative_ancestor_sums(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            report = self._build(
                tmp, db_rows,
                baseline_by_server={"srv01": {"/vault/D/shared-sets/op": 10}})
    ...
    def test_tape_only_and_over_100_percent(self):
        ...
        self.assertEqual(rows["/vault/D/other"]["status"], "partial")
    ...
    def test_high_byte_ratio_without_counts_is_not_full(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            report = self._build(
                tmp, [_db_row("srv01", "/vault/D/other", 216 * GIB)])
        row = self._rows(report)["/vault/D/other"]
    ...
    def test_inode_counts_flow_through_to_full_status(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            report = cov.build_coverage(
                [parse_raw_log(path, server_name="srv01")],
                [_db_row("srv01", "/vault/D/shared-sets/op", 10 * GIB,
                         files=1_000_000)],
                {"srv01": ["/vault/D"]},
                2,
                cov.resolve_host_map(
        ...
        op = self._rows(report)["/vault/D/shared-sets/op"]
    ...
    def test_exact_baseline_overrides_status_end_to_end(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            report = cov.build_coverage(
                [_scan_result(tmp)],
                [_db_row("srv01", "/vault/D/shared-sets/op", 598 * GIB,
                         files=500_000)],
                {"srv01": ["/vault/D"]},
                2,
                cov.resolve_host_map(
                    [ServerConfig("srv01", "srv01.example.edu", "u", "",
                                  ["/vault/D"])]),
                baseline_by_server={
                    "srv01": {"/vault/D/shared-sets/op": 500_000}},
            )
        op = self._rows(report)["/vault/D/shared-sets/op"]
...
class DeletionLedgerTests(unittest.TestCase):
    ...
    def _record(self, path="/vault/E/shared-sets/x", **over):
        rec = {"server": "srv02", "path": path, "deleted_at": "2026-07-26T22:31:00",
    ...
    def test_append_then_load_round_trips(self):
        ...
        self.assertEqual(loaded[0]["path"], "/vault/E/shared-sets/x")
    ...
    def test_totals_sum_files_and_bytes_across_servers(self):
        ...
        deletions_lib.append_record(
            self.smcfg, self._record("/b", files=4, bytes=200, server="srv01"))
        ...
        self.assertEqual(totals["servers"], ["srv01", "srv02"])
...
class DeletionApiTests(unittest.TestCase):
    ...
    def test_ledger_records_are_served(self):
        deletions_lib.append_record(self.smcfg, {
            "server": "srv02", "path": "/vault/E/shared-sets/demo_dir",
    ...
    def test_inventory_download_serves_a_referenced_file(self):
        name = "srv02_demo_dir_20260726_inventory.tsv.gz"
        os.makedirs(deletions_lib.ledger_dir(self.smcfg), exist_ok=True)
        with open(os.path.join(deletions_lib.ledger_dir(self.smcfg), name),
                  "wb") as fh:
            fh.write(b"payload")
        deletions_lib.append_record(self.smcfg, {
            "server": "srv02", "path": "/x", "deleted_at": "2026-07-26T00:00:00",
