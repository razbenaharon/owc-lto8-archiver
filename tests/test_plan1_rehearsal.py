...
SYNTHETIC_TREE = {
    "/strg/pilot": {"files": {"root.bin": 1024},
                    "dirs": ["docs", "empty", "denied", "big"]},
    "/strg/pilot/docs": {"files": {"a.txt": 10, "b with space.txt": 20,
                                   "יוניקוד.txt": 30},
                         "dirs": []},
    "/strg/pilot/empty": {"files": {}, "dirs": []},
    "/strg/pilot/denied": {"files": {}, "dirs": []},
    "/strg/pilot/big": {"files": {"large.iso": 8 * GiB}, "dirs": []},
}
...
class _Rehearsal(unittest.TestCase):
    ...
    def _coordinator(self, source, scan_paths=("/strg/pilot",), **kwargs):
...
class SyntheticDatasetTests(_Rehearsal):
    ...
    def test_an_empty_directory_is_covered_with_no_segment(self):
        ...
        empty = next(d for d in self.db.directories
                     if d["canonical_path"] == "/strg/pilot/empty")
    ...
    def test_an_unreadable_directory_stops_the_run_and_stays_partial(self):
        source = FakeSource(SYNTHETIC_TREE, unreadable={"/strg/pilot/denied"})
        with self.assertRaises(RuntimeError):
            self._coordinator(source).run()
        denied = next(d for d in self.db.directories
                      if d["canonical_path"] == "/strg/pilot/denied")
    ...
    def test_awkward_filenames_survive_into_the_artifact(self):
        ...
        docs = next(d for d in self.db.directories
                    if d["canonical_path"] == "/strg/pilot/docs")
    ...
    def test_a_single_file_scope_is_supported(self):
        ...
        source = FakeSource({"/strg/pilot/one.bin": {"files": {}, "dirs": []}})
        coordinator = self._coordinator(source,
                                        scan_paths=("/strg/pilot/one.bin",))
        coordinator.file_scope_hints = {"/strg/pilot/one.bin"}
...
class FiniteGroupRehearsalTests(unittest.TestCase):
    def test_one_ownership_period_covers_the_whole_group(self):
        ...
        orch.notifier = None
        orch.governor = None
        orch.remote_host = "so02.example"
        orch.remote_session_path = "/strg/pilot"
