...
class _Tmp(unittest.TestCase):
    ...
    def _writer(self, locator=None, **kwargs):
        return JsonlZstArtifactWriter(
            ...
            scan_directory_id=5, scope="/strg/a", **kwargs)
...
class WriterTests(_Tmp):
    def test_a_clean_write_publishes_and_reports_aggregates(self):
        writer = self._write([("/strg/a/f0", 10, 0), ("/strg/a/f1", 20, 1)])
        ...
        self.assertEqual(writer.first_path, "/strg/a/f0")
        self.assertEqual(writer.last_path, "/strg/a/f1")
    ...
    def test_publication_refuses_to_clobber(self):
        self._write([("/strg/a/f", 10, 0)])
        second = self._writer().open()
        second.add(path="/strg/a/other", size=99, ordinal=0)
        ...
        self.assertEqual([e["path"] for e in entries], ["/strg/a/f"])

    def test_publishing_twice_is_idempotent(self):
        writer = self._write([("/strg/a/f", 10, 0)])
    ...
    def test_ordinals_must_ascend(self):
        writer = self._writer().open()
        writer.add(path="/strg/a/f0", size=1, ordinal=5)
        for bad in (5, 4, 0):
            with self.assertRaises(ArtifactError, msg=str(bad)):
                writer.add(path="/strg/a/x", size=1, ordinal=bad)
        writer.abandon()
...
class ReaderTests(_Tmp):
    def test_a_published_segment_reconstructs_its_exact_ordered_list(self):
        entries = [(f"/strg/a/f{i}", i * 10, i) for i in range(50)]
        self._write(entries)
        ...
        self.assertEqual(header["scope"], "/strg/a")
    ...
    def test_optional_hints_round_trip(self):
        with self._writer() as writer:
            writer.add(path="/strg/a/big", size=10 ** 10, ordinal=0,
...
class PathEdgeCaseTests(_Tmp):
    def test_awkward_but_legal_linux_names_round_trip(self):
        names = [
            ...
            "/strg/a/with\ttab",
            "/strg/a/with'quote",
            '/strg/a/with"doublequote',
            "/strg/a/with\\backslash",       # legal on Linux, not a separator
            "/strg/a/יוניקוד",
            "/strg/a/emoji-\U0001F600",
            "/strg/a/" + "x" * 200,
            "/strg/a/trailing.dot.",
        ]
    ...
    def test_a_newline_in_a_name_cannot_break_the_jsonl_framing(self):
        ...
        self._write([("/strg/a/two\nlines", 5, 0)])
        _header, entries, totals = parse_jsonl_zst_artifact(self.root,
                                                            self.locator)
        self.assertEqual(entries[0]["path"], "/strg/a/two\nlines")
