...
distributed across the critical mount points of the lab servers (srv01, srv02),
...
def load_storage_map_config(cfg):
    ...
    if requested_depth != depth:
        print(f"[STORAGE_MAP] scan_depth={requested_depth} is outside the "
              f"supported top-layer/shared-sets view; using scan_depth={depth}.")
    ...
    if not servers:
        raise RuntimeError(
            f"[{CONFIG_SECTION}] defines no servers. Set e.g. 'servers = srv01, srv02'.")
...
def _shared_data_child(mount):
    """Return the shared-sets child path for a configured POSIX mount."""
...
def path_segments(path):
    """``/vault/D`` -> 2 segments; ``/`` -> 0. Used to size the awk rollup."""
...
class MountTree:
    """One mount hierarchy: top layer, plus one extra level in shared-sets."""
...
def _limit_tree_depth(node, max_depth, depth=0):
    """Keep top-level dirs, plus one extra level inside shared-sets."""
...
def build_treemap_figure(results):
    ...
    for res in results:
        ...
        for idx, mt in enumerate(res.mounts):
            ...
            # Label the mount root with its full path (e.g. '/vault/E'), not just
