...
it, with one extra level inside ``shared-sets`` (for example
``/vault/D/shared-sets/op``). The servers hold many TB
...
def resolve_host_map(servers, override=None):
    ...
    (``srv02.example.edu`` -> ``srv02``); explicit ``host_map`` config
