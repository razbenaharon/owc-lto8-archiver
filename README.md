...
- **Storage Map v2 (interactive)** - run `python storage_map/serve.py --open` for a local web dashboard with in-app scan/status/fetch buttons and a **tape-coverage** view matching each server's top-level directories, plus depth 2 inside `shared-sets`, against the archive database
