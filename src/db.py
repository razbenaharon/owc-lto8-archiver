...
def _file_record_key(original_path, tape_label, local_session_id=None,
                     local_chunk_index=None, source_host='srv02'):
...
class DatabaseManager:
    ...
    def _init_schema(self):
        self.conn.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS files_index (
                ...
                tape_label      TEXT,
                source_host     TEXT NOT NULL DEFAULT 'srv02',
                is_packed       BOOLEAN,
                container_name  TEXT,
                stored_path     TEXT,
        ...
        """)
        self.conn.execute(
            "UPDATE files_index SET source_host='srv02' "
    ...
    def _hydrate_file_row(row):
        ...
        item['source_host'] = _short_source_host(item.get('source_host') or 'srv02')
    ...
    def _bulk_upsert_batch(self, records, update_existing):
        ...
        for record in records:
            ...
            source_host = _short_source_host(record.get('source_host') or 'srv02')
                ...
                """UPDATE files_index AS target
                   ...
                   WHERE target.file_id = (
                       ...
                       WHERE COALESCE(legacy.original_path, '') = COALESCE(?, '')
                         AND COALESCE(legacy.source_host, 'srv02') = COALESCE(?, 'srv02')
    ...
    def file_record_exists(self, original_path, tape_label, local_session_id=None,
                           local_chunk_index=None, source_host='srv02'):
        with self.lock:
            source_host = _short_source_host(source_host or 'srv02')
            ...
            return bool(self.conn.execute(
                """SELECT 1 FROM files_index
                   WHERE COALESCE(original_path, '') = COALESCE(?, '')
                     AND COALESCE(source_host, 'srv02') = COALESCE(?, 'srv02')
    ...
    def list_source_hosts(self):
        with self.lock:
            return [row[0] for row in self.conn.execute(
                """SELECT DISTINCT COALESCE(source_host,'srv02') AS source_host
                   FROM files_index
