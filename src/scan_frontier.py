...
def canonicalize_scopes(roots):
    ...
    ``/strg/a`` into ``/strg`` would make per-scope coverage meaningless — the
...
class SegmentChunkPublisher:
    ...
    * **Never silently drop a ``source_changed`` entry.** Same path, different
    ...
    def _drop_unrepresentable(self, segment, pairs):
        ...
        into a forward slash, so ``/strg/a/back\\slash`` and
        ``/strg/a/back/slash`` become the SAME catalog key. On Linux those are
        two different files.
