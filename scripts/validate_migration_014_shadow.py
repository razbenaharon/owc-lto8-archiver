...
def build_legacy_fixture(db, exec_sql):
    ...
    for chunk_index in range(6):
        db.append_remote_streaming_chunk(
            ...
            [(chunk_index, f"/strg/a/c{chunk_index}/f{i}", f"f{i}", 1000 + i)
             for i in range(8)])
    ...
    db.bulk_upsert_files([{
        "original_path": "/strg/a/c0/f0", "file_size_bytes": 1000,
        "tape_label": "SHADOW_LEGACY", "source_host": "so02",
        "is_packed": False, "container_name": None,
        "stored_path": "/strg/a/c0/f0"}])
    return session_id
...
def main(argv=None):
    ...
    try:
        ...
        db.create_scan_scopes(session_id, ["/strg/a"])
        scope = db.get_scan_scopes(session_id)[0]
        db.enqueue_scan_directories(scope["scan_scope_id"], [("/strg/a", 0)])
